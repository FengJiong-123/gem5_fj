...
void mem_test(unsigned int* buffer, unsigned int thread_id, uint op)
{
...
int main(int argc, char *argv[])
{
    ...
    //buffer[1] = (unsigned int *)mmap(0, SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
    ...
    for (int i = 1; i < cpus ; i++) {
        ...
        threads[i] = thread(mem_test, buffer[0], i, op_rd);
    }
